...
y = final['Price']
# Es una serie de tiempo. El split tiene que ser TimeSeriesSplit.
# El entreamiento del modelo no debe ocurrir en la app:
...
# Porque no predices la serie entera hacia el futuro? No en necesario predecir cada día.
...
# Esto se una como input para algo?
...
# Agregale docstrings a tus funciones.
# Si kgs_left se usa como argumento dalo como argumento a la función. Lo mismo con el rf.
def loss_recoup():
    if st.button('Target'):
        row = final[(final['Dia'] == date_input.day) & (final['Mes'] == date_input.month) & (final['Año'] == date_input.year)]
...
# Esta función es demasiado larga. Partela en 3-4 funciones distintas con nombres que explique que hace casa paso.
...
# El usuario no deberia poner estas cantidades?
...
# Estas haciendo un brute force con mucho intentos. Pero no hay una manera para optimizarlo? Creo que con linear programming lo podrias hacer más facil.
